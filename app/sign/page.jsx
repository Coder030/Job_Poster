  ...
  async function makeCookie() {
    try {
      ...
      const response = await fetch(
        "https://test-backend-p5ig.onrender.com/make_cookie",
        {
          method: "POST",
          body: JSON.stringify({
            name: name2,
  ...
  return (
    <>
      ...
      <div className="div1">
        ...
        <Link style={{ textDecoration: "none" }} href="/api/post">
          <button className="button2">New Post</button>
        </Link>
        <Link style={{ textDecoration: "none" }} href="/api/look">
          <button className="button2">Find Jobs</button>
        </Link>
      </div>
    </>
  );
